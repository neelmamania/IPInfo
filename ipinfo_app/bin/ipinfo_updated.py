...
class ExtractDicom(StreamingCommand):
    def stream(self, records):
        ...
        for record in records:
            ...
            if ip_value != "":
                record_list[ip_value + ":::"+str(count1)] = record
                count1 = count1 + 1
                if count1%1000==0:
                    list_of_ips=record_list.keys()
                    request_ip = ', '.join(['"%s"' % w.split(":::")[0] for w in list_of_ips])
                    request_ip = '['+request_ip+']'
                    #logger.error(request_ip)
                    list_of_ip_details = getipinfo(self,request_ip)
                    if(bool(list_of_ip_details)):
                        ...
                        for key in temp.keys():
                            try:
                                record_v1 = temp[key]
                                record_v1.update(list_of_ip_details[key.split(":::")[0]])
        ...
        if count1%1000!=0:
            ...
            if(bool(list_of_ip_details)):
                for key in record_list.keys():
                    ...
                    record_v1.update(list_of_ip_details.get(key.split(":::")[0]))
                    yield record_v1
            else:
                for key in record_list.keys():
                    record_v1 = record_list[key]
...
def getipinfo(object,list_of_ips):
    ...
    try:
        ...
        object.write_warning("Error During Fetching Data from ipinfo.io. Check $SPLUNK_HOME/var/log/splunk/ipinfo/ipinfo.log file for troubleshooting")
        logger.error(e)
        return {}
